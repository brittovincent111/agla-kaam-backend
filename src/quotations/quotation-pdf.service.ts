...
function statusInfo(
  ...
  return {
    ...
    sent: { label: 'SENT', bg: colors.urgencyBg, text: colors.urgencyText },
    accepted: { label: 'ACCEPTED', bg: colors.tintBg, text: colors.tintText },
    rejected: {
      label: 'REJECTED',
      bg: colors.dangerBg,
      text: colors.dangerText,
    },
    converted: { label: 'CONVERTED', bg: colors.tintBg, text: colors.tintText },
  };
}
...
export class QuotationPdfService {
  ...
  async generate(
    ...
    return this.render(business, customer, quotation, templateId);
  }
  ...
  render(
    ...
    const notesHeight = quotation.notes
      ? this.measureTextHeight(quotation.notes)
      : 0;
    const termsHeight = quotation.termsAndConditions
      ? this.measureTextHeight(quotation.termsAndConditions)
      : 0;
    ...
    let y = this.drawTitleRow(doc, quotation, theme);
    y = this.drawBillingInfo(doc, quotation, customer, y, theme);
    y = this.drawItemsTable(doc, quotation, y, theme);
    y = this.drawTotals(doc, quotation, business, y, theme);
    if (quotation.notes) {
      y = this.drawTextBox(
        doc,
        'NOTES',
  ...
  private drawTitleRow(
    ...
    doc
      ...
      .text('QUOTATION', M, top, { characterSpacing: 1.5 });
    doc
      .font('Helvetica-Bold')
      .fontSize(24)
      .fillColor(colors.text)
      .text(quotation.quotationNumber, M, top + 14);

    const status =
      statusInfo(colors)[quotation.status] ?? statusInfo(colors).draft;
  ...
  private drawBillingInfo(
    ...
    doc
      ...
      .fontSize(9)
      .fillColor(colors.textMuted)
      .text('QUOTED TO', M, top, { characterSpacing: 1 });
    doc
      .font('Helvetica-Bold')
      .fontSize(13)
      .fillColor(colors.text)
  ...
  private drawTotals(
    ...
    row('Subtotal', formatCurrency(quotation.subtotal));
    if (quotation.discount > 0)
      row('Discount', `- ${formatCurrency(quotation.discount)}`);
    if (business.gstin) {
      row('CGST', formatCurrency(quotation.taxTotal / 2));
      row('SGST', formatCurrency(quotation.taxTotal / 2));
    } else {
      row('Tax', formatCurrency(quotation.taxTotal));
    }
    ...
    row('Total', formatCurrency(quotation.total), {
      bold: true,
